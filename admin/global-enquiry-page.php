...
if ( ! class_exists( 'Global_Enquiry_Table' ) ) {
	

	class Global_Enquiry_Table {
		...
		public function pmc_add_plugin_page()
		{
			...
			add_menu_page( esc_attr__( 'Global Enquiry', 'textdomain' ),esc_html__( 'Global Enquiry', 'textdomain' ),
			'administrator','global-enquiry',array( $this, 'physio_create_enquiry_page' ), '', 10);
		...
		public function physio_create_enquiry_page()
		{
			...
			<?php
				$pmc_fs_table = new Global_Enquiry_Table_Content();
				echo '<div class="wrap"><h2>Global Enquiry</h2>'; 
	...
	class Global_Enquiry_Table_Content extends WP_List_Table 
	{
		...
		function prepare_items() {
			...
			$tableEnquiry = $wpdb->prefix . 'physiobrite_global_client_enquiries';

			$items = $wpdb->get_results( "SELECT * FROM {$tableEnquiry} WHERE 1=1 {$search} {$search_custom_vars}" . $wpdb->prepare( "ORDER BY id DESC LIMIT %d OFFSET %d;", $per_page, $offset ),ARRAY_A);
  ...
  $Global_Enquiry_Table = new Global_Enquiry_Table();
