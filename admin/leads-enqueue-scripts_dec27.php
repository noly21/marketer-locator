...
function physiobrite_scripts() {
	global $post;
	if( is_page() ){
		if( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'leadsDashboard') || has_shortcode( $post->post_content, 'leadsInfo') || has_shortcode( $post->post_content, 'register_leads_page') || has_shortcode( $post->post_content, 'emailtry_shortcode') || has_shortcode( $post->post_content, 'show_physio_category') || has_shortcode( $post->post_content, 'show_contact_form') || has_shortcode( $post->post_content, 'register_leads_page_v2') || has_shortcode( $post->post_content, 'hcpc_form') || has_shortcode( $post->post_content, 'renew_account') ) {
...
function physiobrite_style() {
			...
			global $post;
			if( is_page() ){
				if( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'leadsDashboard') || has_shortcode( $post->post_content, 'leadsInfo') || has_shortcode( $post->post_content, 'register_leads_page') || has_shortcode( $post->post_content, 'show_featured_physio') || has_shortcode( $post->post_content, 'show_physio_category') || has_shortcode( $post->post_content, 'show_contact_form') || has_shortcode( $post->post_content, 'register_leads_page_v2') || has_shortcode( $post->post_content, 'hcpc_form') || has_shortcode( $post->post_content, 'renew_account') ) {
...
function add_paypal_script_func(){
  global $post;
    ...
    echo '<script src="https://www.paypal.com/sdk/js?client-id=AR9ht6ylpwNE1fUFKdBjwk71oc_hGWJGuP5FQuP_y-j9B-ZEWvGXYcpY3cQbgFJn9pKuA_sfTlS6hnQM&currency=GBP&components=buttons"></script>';
  	}
   }
}
