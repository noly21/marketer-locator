...
add_action( 'referrerLeads', 'referrer_leads_func');
function referrer_leads_func(){
...

    {
        ...
        $userdata = array(
            ...
            'last_name'             => $lastname,   //(string) The user's last name. For new users, will be used to build the second part of the user's display name if $display_name is not specified.
            'show_admin_bar_front'  => 'false',   //(string|bool) Whether to display the Admin Bar for the user on the site's front end. Default true.
            'role'                  => 'referrer_leads',   //(string) User's role.
        ...
        if($user_id){
            ...
            if (isset($_POST['inputPaypalId']))
                update_user_meta($user_id,'paypal_id', $_POST['inputPaypalId']);
...
function physiobrite_send_enquiries_func() {
    ...
    if (isset($_POST['physioCat']) && isset($_POST['otherNameinfo'])) {
            $physioCat = $_POST['physioCat']; //Physio Cat
		$otherNameinfogender = $_POST['otherNameinfo'];
		
	$physioCat2_cat =  substr(strrchr($physioCat, $needle), 1 ) ;
  $physioCat2gender = strstr($physioCat, $needle, true); 
		
		$physioCat2_cat_number  = preg_replace('/\D/', '', $physioCat);
	
		$physioCatcat_new_male = 'Male,';
		$physioCatcat_new_female = 'Female,';
		$physioCatcat_commer = ',';
		
		if ($physioCat2gender != 'Both'){
			$clientGender = $physioCat2gender; 
			
		}
          else if ($physioCat2gender == 'Both'){
			  $clientGender = $otherNameinfogender ; //Client Gender
			  $physioCat_male = $physioCatcat_new_male.$physioCat2_cat_number.$physioCatcat_commer.$physioCat2_cat ;
			$physioCat_female = $physioCatcat_new_female.$physioCat2_cat_number.$physioCatcat_commer.$physioCat2_cat;
		  }
    ...
    if (isset($_POST['clientOtherContact']))
	   ...
	   if (isset($_POST['clientype']))
            $clientype = $_POST['clientype']; //Client Gender otherName
	
	 if (isset($_POST['otherName']))
            $otherName = $_POST['otherName']; 
	
	if (isset($_POST['otherName']))
        $otherName_1 = $_POST['otherName'];
	
	if (empty ($otherName_1)) {
		$otherName_1 = 'Self Funded';
	}

    if (isset($_POST['serviceType']))
            $serviceType = $_POST['serviceType']; //Client Gender
	
	if (isset($_POST['is_viewed']))
            $isviewed = $_POST['is_viewed']; //Client Gender

     if (isset($_POST['lead_type']))
        {
            $lead_type = $_POST['lead_type']; //Client Gender 
        }
     else{
         $lead_type = 'lead';
     }              
	
	 if (isset($_POST['lead_type1']))
        {
            $lead_type1 = $_POST['lead_type1']; //Client Gender 
        }
     else{
         $lead_type1 = 'Enquiry';
     }   
    ...
    $data = array(
                'physio_id' => $physioId,
		'referrer_id' => $physioId,
                'physio_cat' => $physioCat,
		'physio_cat_male' => $physioCat_male,
		'physio_cat_female' => $physioCat_female,
		'physio_cat_2' => $physioCat2_cat,
		'preferred_gender' => $otherNameinfogender,
		...
		'other_Name' => $otherName_1,
		 'is_viewed' => $isviewed,
                'lead_type' => $lead_type,
		 'lead_type1' => $lead_type1,
                'client_problem' => '',
                'date_created' => current_time('mysql', 1)
            );

    $format = array('%d','%d','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');

    $insertId = $wpdb->insert($table,$data,$format);

    if($lead_type == 'referrerLead'){
         if ( $insertId ) {
            swal_message('success', 'Success!', 'Thank you');
        }   
    }else{
        if ( $insertId && sendEnquiryEmail($data)) {
            global $wp;
            swal_message('success', 'Success!', 'Your selected physiotherapist will be in contact with you shortly. Should they not be in contact with you within 24 hours, another physiotherapist may contact you. Thank you.');
        }else{
    ...
    if ( $viewLeads->is_viewed == 0 ){
        ...
        if( $currentLeadsCredit > 0 && $currentLeadsCredit >= 10 ){
            ...
            if($updateCredit){
                ...
                $data = array( 'is_viewed' => 1, 'is_approached' => 1 );
                $where = array( 'ID' => $leadsId );
...
add_action('wp_ajax_data_lead_table', 'data_lead_table');
add_action('wp_ajax_nopriv_data_lead_table', 'data_lead_table');
function data_lead_table() {
    ob_start();
    global $wpdb;
    ...
    $sql = "SELECT * FROM ". $tbl_name ." WHERE referrer_id = '" . $userId . "' AND lead_type = 'referrerLead'";
    if( !empty($requestData['search']['value']) ) 
     {
        ...
        $sql.=" OR  service_type LIKE '".$requestData['search']['value']."%' )";
		  $sql.=" OR  other_Name LIKE '".$requestData['search']['value']."%' )";
		  $sql.=" OR  physio_cat_2 LIKE '".$requestData['search']['value']."%' )";
        ...
        if( count( $listEnquiry ) > 0 ){
            ...
            foreach ($dataEnquiry as $enquiryData) {
                    ...
                    $enquiryData->client_gender,
						...
						$enquiryData->service_type,
						$enquiryData->other_Name,
						$enquiryData->physio_cat_2,
			
                    
				
                    '<button class="btn btn-secondary btn-sm access-enquiry" data-fieldinfo=\''.htmlspecialchars(wp_json_encode($enquiryData), ENT_QUOTES, "UTF-8").'\'" data-id="'.$enquiryData->id.'" data-expanded="false" data-info-loaded="false"><span class="spinner-border spinner-border-sm d-none"></span> Details</button>'
                    );
...
function data_enquiry_table() {
    ...
    if($requestData['leadType'] == 'referrerLead'){
         $enquiryList = $wpdb->get_results($wpdb->prepare(
                    "SELECT * FROM ". $tbl_name ." WHERE lead_type = 'referrerLead'"));
    }else{
         $enquiryList = $wpdb->get_results($wpdb->prepare(
                    "SELECT * FROM ". $tbl_name ." WHERE physio_id = %d", 
                    $userId
    ...
    //Only get data that have date diff less than 31
    if($requestData['leadType'] == 'referrerLead'){
        $sql = "SELECT * FROM ". $tbl_name ." WHERE lead_type = 'referrerLead'";
    }else{
    ...
    if( !empty($requestData['search']['value']) ) 
     {
        ...
        $sql.=" AND ( physio_cat2 LIKE '".$requestData['search']['value']."%' "; 
        //$sql.=" OR  client_name LIKE '".$requestData['search']['value']."%' "; 
        $sql.=" OR  client_address LIKE '".$requestData['search']['value']."%' "; 
        $sql.=" OR  client_gender LIKE '".$requestData['search']['value']."%' ";
        $sql.=" OR  service_type LIKE '".$requestData['search']['value']."%' )";
		 ...
		 $sql.=" OR  lead_type1 LIKE '".$requestData['search']['value']."%' )";
     }
        ...
        if( count( $listEnquiry ) > 0 ){
            ...
            foreach ($dataEnquiry as $enquiryData) {
                ...
                if( $currentLeadsCredit > 0 && $currentLeadsCredit >= 10 || $enquiryData->is_viewed == 1 ){
					...
					if($enquiryData->is_viewed == 1  && $enquiryData->is_approached == 0  && $enquiryData->lead_type1 == 'Enquiry' ) {
                    ...
                    $enquiryData->client_gender,
						 ...
						 $enquiryData->other_Name,
                    $enquiryData->physio_cat_2,
							$enquiryData->lead_type1,
						
						
                 
						
						'<button class="btn btn-success2 btn-sm client-approched" data-fieldinfo=\''.htmlspecialchars(wp_json_encode($enquiryData), ENT_QUOTES, "UTF-8").'\'" data-id="'.$enquiryData->id.'" data-expanded="false" data-info-loaded="false" ><span class="spinner-border spinner-border-sm d-none"></span> ACCEPT LEAD</button>',
						  
                    );
}
					
					
			else if($enquiryData->is_viewed == 1  && $enquiryData->is_approached == 1 ) {
                    ...
                    '<button class="btn btn-success1 btn-sm access-enquiry" data-fieldinfo=\''.htmlspecialchars(wp_json_encode($enquiryData), ENT_QUOTES, "UTF-8").'\'" data-id="'.$enquiryData->id.'" data-expanded="false" data-info-loaded="false" ><span class="spinner-border spinner-border-sm d-none"></span>  DETAILS</button>',
                    ...
                    '<button class="btn btn-success btn-sm access-enquiry" data-fieldinfo=\''.htmlspecialchars(wp_json_encode($enquiryData), ENT_QUOTES, "UTF-8").'\'" data-id="'.$enquiryData->id.'" data-expanded="false" data-info-loaded="false" ><span class="spinner-border spinner-border-sm d-none"></span> BUY NOW</button>',
...
add_action('wp_ajax_data_enquiry_tableall', 'data_enquiry_tableall');
add_action('wp_ajax_nopriv_data_enquiry_tableall', 'data_enquiry_tableall');
function data_enquiry_tableall() {
    ob_start();
    global $wpdb;
    $userId = get_current_user_id();
    $requestData= $_REQUEST;
    ...
    $currentDateTime = current_time('mysql', 1);
    //Only get data that have date diff less than 31
    if(!empty($requestData['leadType'] )){
        $sql = "SELECT * FROM ". $tbl_name ." WHERE  is_approached = '1' ";
    }else{
        $sql = "SELECT * FROM ". $tbl_name ." WHERE is_approached = '1' ";
    }
    if( !empty($requestData['search']['value']) ) 
...
function update_leads2_func(){
    ...
    $staffInfo = array();
    if ( isset( $_POST['inputStaffName'] ) && isset( $_POST['inputStaffHcpc'] ) && isset( $_POST['inputStaffEmail'] ) && isset( $_POST['inputStaffgender'] )  ){
       foreach ($_POST["inputStaffName"] as $key => $value) {
            $item = $value . ',' . $_POST["inputStaffEmail"][$key] . ',' . $_POST["inputStaffHcpc"][$key] . ',' . $_POST["inputStaffgender"][$key];
    ...
    if( $user_pass !== $user_pass_check ){
        ...
        if (isset($_POST['inputFirstName']) || isset($_POST['inputLastName'])){
            ...
            if (isset($_POST['inputGendermain'])){
                $postGender = sanitize_text_field($_POST['inputGendermain']);
                update_post_meta($post_id, 'store_locator_gender_main', $postGender);
            }
			
			if (isset($_POST['inputservicetypemain'])){
                $postGender = sanitize_text_field($_POST['inputservicetypemain']);
                update_post_meta($post_id, 'store_locator_service_type', $postGender);
            }
			...
			// kelly here for category edits - physio edit cat 25/05/2020
		...
		if(!empty($post_id)){
			...
			$physiobrite_featured_physio = $wpdb->prefix.'physiobrite_featured_physio';
    
					
			$wpdb->query( $wpdb->prepare("UPDATE $physiobrite_featured_physio 
                SET store_locator_lat = %s ,
                store_locator_lng = %s ,
                physio_city = %s ,
                service_type = %s 
             WHERE leadsId = %d",$_POST['store_locator_lat'],$_POST['store_locator_lng'], $_POST['store_locator_city'],$_POST['inputservicetypemain'], $post_id)
			);
    ...
    $values = $_POST['inputGender'];
				$needle    = ',';
					

		 delete_post_meta( $post_id, 'store_locator_gender_category' );	
					
			wp_delete_object_term_relationships( $post_id, 'store_locator_category');	
					
				
    foreach( $values as $value ) {
		add_post_meta( $post_id, 'store_locator_gender_category', $value );
		
		
		$specialitynew_65a_selected_gender =  strstr( $value, $needle, true );   
	
		
				
	
				
				if ($specialitynew_65a_selected_gender == 'none') {
					
					$post_categories_65_update = array();
				...
				$table_taxonomy_results_numbers_65_update_first = preg_replace('/\D/', '', $value);
					$post_categories_65_update = array ($table_taxonomy_results_numbers_65_update_first );
		 $post_ID     = (int) $store_id;
    $post_type  = get_post_type( $store_id );
    $post_status= get_post_status( $store_id );
    // If $post_categories isn't already an array, make it one:
    $post_categories_65_update = (array) $post_categories_65_update;
								 							 
    if ( empty( $post_categories_65_update ) ) {
        if ( 'post' == $post_type && 'auto-draft' != $post_status ) {
            $post_categories_65_update = array( get_option( 'default_category' ) );
            
        } else {
            $post_categories_65_update = array();
        }
    } elseif ( 1 == count( $post_categories_65_update ) && '' == reset( $post_categories_65_update ) ) {
        return true;
    }
 
     wp_set_post_terms( $post_id, $post_categories_65_update, 'store_locator_category', 'false' );
			...
			// kelly here for category edits combine this and the above so it does it in one go
			

								 	$post_categories_66 = array ($table_taxonomy_results_numbers_65_update, 
																 $table_taxonomy_results_numbers_66_update, 
																  );
    ...
    $post_status= get_post_status( $store_id );
    // If $post_categories isn't already an array, make it one:
    $post_categories_66 = (array) $post_categories_66;
								 
								 
    if ( empty( $post_categories_66 ) ) {
        if ( 'post' == $post_type && 'auto-draft' != $post_status ) {
            $post_categories_66 = array( get_option( 'default_category' ) );
            
        } else {
            $post_categories_66 = array();
        }
    } elseif ( 1 == count( $post_categories_66 ) && '' == reset( $post_categories_66 ) ) {
        return true;
    }
 
     wp_set_post_terms( $post_ID, $post_categories_66, 'store_locator_category', 'true' );
...
add_action( 'updateLeads2kel', 'update_leads2kel_func'); //kell
function update_leads2kel_func(){
    global $wpdb;
...
function psyb_paypal_transaction(){
	...
	if (isset($_POST['store_locator_lat']))
            $store_locator_lat = $_POST['store_locator_lat'];
		
	if (isset($_POST['store_locator_lng']))
            $store_locator_lng = $_POST['store_locator_lng'];
    ...
    if( $transaction_type == 'upgrade_account' ){
        ...
        $insertId = $wpdb->insert($table,$data,$format);
      	...
      	$dataPromoted = array(
                    	...
                    	'date_created'  	=> current_time('mysql', 1),
						'store_locator_lat' => $store_locator_lat ,
						'store_locator_lng' => $store_locator_lng,
						'leadsId' => $leadsId,
                	);

        $formatPromoted = array('%s','%s','%d','%s','%s'  , '%s', '%s', '%d');
        ...
        $insertIdPromoted = $wpdb->insert($tablePromoted,$dataPromoted,$formatPromoted);
          ...
          'SELECT *, DATE_ADD(date_created, INTERVAL 3 MONTH) as date_expired FROM '. $tbl_featured . ' WHERE physio_id = %d',
          $physio_id
        ));
...
function physiobrite_send_global_enquiries_func() {
    ...
    if (isset($_POST['physioCat']) && isset($_POST['clientGender'])) {
		
		
		$needle    = ',';
		$physioCatgender = $_POST['clientGender'];
		$physioCatcat_new = $_POST['physioCat'];
		$physioCatcat_new_male = 'Male';
		$physioCatcat_new_female = 'Female';
		
		
		if ($physioCatgender == 'Both' ) {
			
			$physioCat = $physioCatgender.$physioCatcat_new;
			$physioCat_male = $physioCatcat_new_male.$physioCatcat_new ;
			$physioCat_female = $physioCatcat_new_female.$physioCatcat_new;
	...
	if (  is_null($_POST['lead_type']) )
	
{
 update_leads2kel_func();}
	else {
...
if (isset($_POST['lead_type']))
    ...
    //Save the Inquiry
    $format = array('%d','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s'); //kelly
    ...
    if($lead_type == 'referrerLead'){
        ...
        if ( $insertId && sendEnquiryEmail($data)) {
            ...
            swal_message('error', 'Error!', 'There was an error in sending your Enquiry. Please refresh the page and try again.1');
        }
     }
}
...
function data_global_enquiry_table() {
	...
	$currentLeadsuserid= get_user_meta( $userId, 'leads_id', true );
	
	
	// if gender doesn't = both do the below
    ...
    //Only get data that have date diff less than 31
         ...
         $enquiryList = $wpdb->get_results($wpdb->prepare(
                    "SELECT * FROM ". $tbl_name ." WHERE physio_id is NULL", 
                    $currentLeadsuserid
                  ));
   
	
        $sql = "SELECT * FROM ". $tbl_name ." WHERE physio_id IS NULL";
    ...
    if( !empty($requestData['search']['value']) ) 
     {
		...
		$sql.=" OR  physio_cat_2 LIKE '".$requestData['search']['value']."%' )";
		$sql.=" OR  lead_type1 LIKE '".$requestData['search']['value']."%' )";
		$sql.=" OR  preferred_gender LIKE '".$requestData['search']['value']."%' )";
        ...
        if( count( $listEnquiry ) > 0 ){
            ...
            foreach ($dataEnquiry as $enquiryData) {
                if( $currentLeadsCredit > 0 && $currentLeadsCredit >= 10 || $enquiryData->is_viewed == 1){
					 ...
					 $currentLeadsuserid_cat_65= get_post_meta( $currentLeadsuserid, 'store_locator_gender_category', false );
					foreach ($currentLeadsuserid_cat_65 as $currentLeadsuserid_cat_65_value) {
						
						$needle    = ',';
						$specialitynew_65a_selected_gender =  strstr( $currentLeadsuserid_cat_65_value, $needle, true );   
						$specialitynew_65a_selected_cat =   substr(strrchr( $currentLeadsuserid_cat_65_value, $needle), 1 ); 
						
						
						
					if (($enquiryData->physio_cat == $currentLeadsuserid_cat_65_value) || ($enquiryData->physio_cat_male == $currentLeadsuserid_cat_65_value) || ($enquiryData->physio_cat_female == $currentLeadsuserid_cat_65_value)   ) {
...
function psyb_move_enquiry() {
    ...
    $data = array(
                'physio_id'             => $userId,
		'referrer_id'	=> $globalEnquiryInfo->referrer_id,
		...
		'physio_cat_2'          => $globalEnquiryInfo->physio_cat_2,
		'preferred_gender'          => $globalEnquiryInfo->preferred_gender,
		...
		'client_type'         => $globalEnquiryInfo->client_type,
		'other_Name'         => $globalEnquiryInfo->other_Name,
                'client_problem'        => $globalEnquiryInfo->client_problem,
		'lead_type' 	=> $globalEnquiryInfo->lead_type,
		'lead_type1' 	=> $globalEnquiryInfo->lead_type1,
...
function lookForDuplicateHCPC( $hcpc_id = '' ){
    ...
    if ($hcpc_id !== '') {
        ...
        $data = $wpdb->get_results( $wpdb->prepare( "SELECT hcpc_id FROM $table_unverified WHERE hcpc_id = %s NOT Referrer", $hcpc_id ), true );
        $result = $wpdb->get_results( $wpdb->prepare( "SELECT hcpc_id FROM $table_usermeta WHERE meta_value = %s NOT Referrer", $hcpc_id ), true );
...
function register_service_provider_func(){
	...
	if (isset($_POST['post_type_un']))
        $post_type_un = sanitize_text_field($_POST['post_type_un']);
	
	if (isset($_POST['post_name_un']))
        $post_name_un = sanitize_text_field($_POST['post_name_un']);

    $staffInfo = array();
    if ( isset( $_POST['inputStaffName'] ) && isset( $_POST['inputStaffEmail'] ) && isset( $_POST['inputStaffHcpc'] ) && isset( $_POST['inputStaffgender'] ) ){
       foreach ($_POST["inputStaffName"] as $key => $value) {
            $item = $value . ',' . $_POST["inputStaffEmail"][$key] . ',' . $_POST["inputStaffHcpc"][$key] . ',' . $_POST["inputStaffgender"][$key];
            array_push($staffInfo, $item);
       }
    ...
    */
...

    {
        ...
        $data = array(
                    ...
                    'hcpc_id'       => $hcpc_id,
                    'role'          => $user_role,
			'post_type_un'          => $post_type_un,
			'post_name_un'          => $post_name_un,
                    'date_created'  => current_time('mysql', 1) 
                );
	...
	// if (isset($_POST['inputGender'])){
      //          $postGender = sanitize_text_field($_POST['inputGender']);
     //         update_post_meta($post_id, 'store_locator_gender', $postGender); // store_locator_gender  database name for search but now category gender
     //      }
	
	// if (isset($_POST['inputGenderCategory'])){
       //        $postGender = sanitize_text_field($_POST['inputGenderCategory']);
      //       update_post_meta($post_id, 'store_locator_gender_category', $postGender); 
      //    }	
			
			// need to input an arry into database
    ...
    if ( empty( $post_categories_65_update ) ) {
     ...
     wp_set_post_terms( $post_id, $post_categories_65_update, 'store_locator_category', 'false' );
			...
			if (isset($_POST['user_role'])){
                $postuserrole = sanitize_text_field($_POST['user_role']);
                update_post_meta($post_id, 'store_locator_role',  $postuserrole);
            }
			
			if (isset($_POST['post_type_un'])){
                $pospost_type_un = sanitize_text_field($_POST['post_type_un']);
                update_post_meta($post_id, 'store_locator_role_post_type_un',  $pospost_type_un);
            }
			
			if (isset($_POST['post_name_un'])){
                $pospost_name_un= sanitize_text_field($_POST['post_name_un']);
                update_post_meta($post_id, 'store_locator_role_post_name_un',  $pospost_name_un);
            }
            ...
            if (isset($_POST['store_locator_zipcode'])){
			    ...
			    if (isset($_POST['inputservicetypemain'])){
                $postservice = sanitize_text_field($_POST['inputservicetypemain']);
              update_post_meta($post_id, 'store_locator_service_type', $postservice); 
           }
    ...
    if ( isset($_POST['store_id']) ):
       ...
       // $hcpc_id = $_POST['hcpc_id'];
        ...
        if ( $currentUserData->is_verified == "0" ) :
            ...
            if( $updated ):
                swal_message('success', 'Success!', 'Thank you for verifying your email account. Once a member of our admin staff have activated your account an email will be sent to you containing your Username and Password.' );
                echo "<p class='text-center pt-3'>Thank you for verifying your email account. Once a member of our admin staff have activated your account an email will be sent to you containing your Username and Password.</p>";
            endif;
        else:
    ...
    if ( isset($_GET['store_id']) ):
        ...
        if( $updatePost ):
            ...
            $userdata = array(
				...
				'post_type_un'                  => $currentUser->post_type_un,
                    );
...
add_action( 'admin_post_verify_user_account1', 'verify_user_account1_admin_action' ); 
function verify_user_account1_admin_action(){
    global $wpdb;
