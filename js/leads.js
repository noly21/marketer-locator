...
//Inquiry table
	$('#leadEnquiryTable .view-info').on('click', function(){
		$('#enquiryInfo').modal('show')
	});

	$('#leadEnquiryTable').on('click', '.access-enquiry', function(){
		...
		$('#leadEnquiryTable').find('button').removeAttr('disabled');
		$(document).find('#enquiryInfo').modal('show');

	});

	//Inquiry table
	var leadEnquiryTable = $('#leadEnquiryTable').DataTable({
		"processing": true,
		"serverSide": true,
		//"order": [[ 0, "desc" ]],
		//"scrollX": true,
		...
		"drawCallback": function( settings ) {
			if( settings.json.credits == 0 || settings.json.credits < 10 ){
				$( '#leadEnquiryTable tr' ).each( function( index ) {
					var currRow = $(this);
	...
	//Inquiry table
	var referrerLeadTable = $('#referrerLeadTable').DataTable({
		"processing": true,
		"serverSide": true,
		//"order": [[ 0, "desc" ]],
		//"scrollX": true,
	...
	//Inquiry table
	$('#enquiryTable .view-info').on('click', function(){ //kell removed ,'#referrerLeadTable .view-info'
		$('#enquiryInfo').modal('show')
	});
...
$(document).ready(function() {
   ...
   $("#otherNameinfo").hide();
	...
	$("#clientype").on("change", function() {
        if ($(this).val() === "Private Insurance") {
            $("#otherName").show();
        }
        else {
            $("#otherName").hide();
	...
	$("#physioCat").on("change", function() {
        if ($(this).val().substring(0,4) === "Both") {
            $("#otherNameinfo").show();
        }
        else {
            $("#otherNameinfo").hide();
	...
	//Inquiry table
	$('#enquiryTableall .view-info').on('click', function(){ //kell removed ,'#referrerLeadTable .view-info'
		$('#enquiryInfo').modal('show')
	});

	//Inquiry table
	var enquiryTableall = $('#enquiryTableall').DataTable({
		"processing": true,
		"serverSide": true,
		//"order": [[ 0, "desc" ]],
		//"scrollX": true,
		...
		"ajax":{
			...
			data: function(d) {
				if(d){
					d.action = 'data_enquiry_tableall';
				}
			}
		},
		...
		"drawCallback": function( settings ) {
			if( settings.json.credits == 0 || settings.json.credits < 10 ){
				$( '#enquiryTableall tr' ).each( function( index ) {
					var currRow = $(this);
					var inquiryVal = $(this).find('.access-enquiry').data('fieldinfo');
	...
	$('#enquiryTable').on('click', '.client-approched', function(){ //kell removed ,#referrerLeadTable
		var leadsId = $(this).data('id');
		var tableInquiry = $(this);
		var dataInfo = $(this).data('fieldinfo');
	...
	$('#enquiryTable, #referrerLeadTable').on('click', '.access-enquiry', function(){ //kell removed ,#referrerLeadTable
				...
				if (response.success == true) {
					if( response.is_deducted == true ){
						...
						if ( response.remainingCredit == 0 || response.remainingCredit < 10 ) {
							...
							$( '#enquiryTable .access-enquiry,#referrerLeadTable  .access-enquiry' ).each( function( index ) { //kell removed ,#referrerLeadTable  .access-enquiry
								var thisBtn = $(this);
					...
					(async function getAmount () {
					    const {value: chargeAmount} = await Swal.fire({
					      ...
					      text: 'Enter amount: GBP - Each Lead = 10 Credits and £10 = 10 credits.',
					      input: 'number',
					      showCancelButton: true,
	...
	function modalInfo(obj){
		...
		$(document).find('#enquiryInfo .client-category p').html('<strong>Speciality Required: </strong>' + obj.physio_cat_2);
		...
		$(document).find('#enquiryInfo .client-type p').html('<strong>Client Type: </strong>' + obj.client_type);
	...
	$('#store_locator_country').on('change', function(){
		...
		//Leads Dashboard1
	$('#dashboard-edit-form1 .leads-info-form').addClass('d-none');

	$('#dashboard-edit-form1').on('click', '.edit-form', function(){
		$('#dashboard-edit-form1 .leads-info-form').removeClass('d-none');
		$('#dashboard-edit-form1 .leads-info-col').addClass('d-none');
	});

	$('#dashboard-edit-form1').on('click', '.cancel-form', function(){
		$('#dashboard-edit-form1 .leads-info-form').addClass('d-none');
		$('#dashboard-edit-form1 .leads-info-col').removeClass('d-none');
	});
		...
		if( fname.val() == "" || lname.val() == "" || email.val() == "" || phone.val() == "" || gender.val() == "Choose..." || cat.val() == "" || username.val() == "" || userpass.val() == "" || locator_address.val() == "" || locator_country.val() == "" || locator_city.val() == "" || locator_zipcode.val() == "" ){
			if (Swal.isVisible() == false) {
				Swal.fire({
				  type: 'error',
				  title: 'Error!',
				  text: 'Please fill in all of the information in the form'
				});
				  formError = 1;
			}
		}
	...
	/*===================Service Provider Update1===================*/
	$(document).on('submit', '#dashboard-edit-form1', function(e){

		var formError = 0;
		var worktimeError = 0;

		var form = $('#dashboard-edit-form1');
		var fname = form.find('[name="inputFirstName"]');
		var lname = form.find('[name="inputLastName"]');
		var email = form.find('[name="inputEmail"]');
		var phone = form.find('[name="inputPhone"]');
		...
		var matchPass = $('#dashboard-edit-form1 .response_password_check .text-danger ').length;
		...
		if( fname.val() == "" || lname.val() == "" || email.val() == "" || phone.val() == "" || username.val() == "" || userpass.val() == "" || locator_address.val() == "" || locator_country.val() == "" || locator_city.val() == "" || locator_zipcode.val() == "" ){
			if (Swal.isVisible() == false) {
				Swal.fire({
				  type: 'error',
				  title: 'Error!',
	...
	$(document).on('submit', '#book_form', function(e){
		...
		var clientype = form.find('[name="clientype"]');
		var termsCheck = form.find('[name="termsCheck"]');
		var serviceType = form.find('[name="serviceType"]');

		if( clientName.val() == '' || clientAddress.val() == '' || clientTel.val() == '' || clientEmail.val() == '' || clientTimeCall.val() == '' || physioCat.val() == 'Speciality' || clientGender.val() == 'Gender Of Physio Required' || clientype.val() == 'Type Of Treatment Payment')
		{
		...
		if ( clientype.val() == 'Type Of Treatment Payment' ) {
			clientype.addClass('input-danger');
			formError = 1
		}
		
		if ( clientGender.val() == 'Gender Of Physio Required' ) {
		...
		clientype.on('focus', function(){
			clientype.removeClass('input-danger');
		});
		...
		if( ($('#termsCheck').is(':checked')) == false){
			if (Swal.isVisible() == false) {
				Swal.fire({
				  type: 'info',
				  title: 'Good day!',
				  text: 'Please agree to our Terms & Conditions should you wish to use our services. Thank you'
				});
			}
			formError = 1;
		}
	...
	$(document).on('submit', '#registerServiceProviderForm', function(e){
		...
		// var phone = form.find('[name="inputPhone"]');
		var gender = form.find('[name="inputGendermain"]');
		var cat = form.find('#inputCategory');
		var username = form.find('[name="username"]');
		//var userpass = form.find('[name="userPass"]');
		var locator_address = form.find('[name="store_locator_address"]');
		...
		var locator_city = form.find('[name="store_locator_city"]');
		var locator_zipcode = form.find('[name="store_locator_zipcode"]');
		var service = form.find('[name="inputservicetypemain"]');
		var locator_days = form.find('[name="store_locator_days"]');
		...
		var staff_members_gender = form.find('.inputStaffgender');
		...
		service.on('focus', function(){
			service.removeClass('input-danger');
		});
		...
		staff_members_gender.on('focus', function(){
			staff_members_gender.removeClass('input-danger');
		});
	
		
		var staff_error = 0;
		staff_members_gender.each(function(){
			if( $(this).val() == "" ){
				staff_error = 1;
				$(this).addClass('input-danger');
				formError = 1;
		...
		if( fname.val() == "" || lname.val() == "" || email.val() == "" || hcpc_id.val() == "" || gender.val() == "Choose..." || cat.val() == "" || username.val() == "" || locator_address.val() == "" || locator_country.val() == "" || locator_city.val() == "" || locator_zipcode.val() == "" || service.val() == "none" || staff_error == 1){
			if (Swal.isVisible() == false) {
				Swal.fire({
				  type: 'error',
				  title: 'Error!',
	...
	$( '.dashboard_container' ).on( 'click', '#upgrade_account', function(e){
		...
		$('#upgradeAccountInfo').modal('show');
		 var currUserId = $( this ).data('id');
	        var leadsId = jQuery("input#leadsId").val();;
	...
	paypal.Buttons({
	    ...
	    onApprove: function(data, actions) {
	      return actions.order.capture().then(function(details) {
            ...
            var store_locator_lat = jQuery('input#store_locator_lat').val();
            var store_locator_lng = jQuery( 'input#store_locator_lng' ).val();
			var leadsId = jQuery("input#leadsId").val();;
	        console.log(details);
	        var saveData = {
				'action' 			: 'psyb_paypal_transaction',
...
$(".staff-section").on("click", ".register-add-member", function(){
	...
	button.text("Add More Members");
	button.parent().append(`<div class="form-row align-items-end" style="border-bottom: #818181 dotted 1px">
...
<div class="form-group col-md-10">
									<label for="inputStaffgender" class="reg-label">Physio Gender<span class="text-danger required">*</span> <em>- please select the gender that the physio identify's with or that most represents the physio</em></label>
									<select id="inputStaffgender" name="inputStaffgender[]" class="form-control">
